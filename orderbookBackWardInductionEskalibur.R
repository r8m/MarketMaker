...
{   
    ...
    for (t in seq(along.with=obMPdf$TT))
    {
        ...
        for (s in seq(along.with=obMPdf$SS))
        {
            ...
            for (y in seq(along.with=obMPdf$YY))
            {
                ...
                for (f in seq(along.with=obMPdf$FF))
                {
                    ...
                    if (t == obMPdf$NT) 
                        w[t, y, f,s] <<- -abs(obMPdf$YY[y]) * (obMPdf$SS[s] * obMPdf$deltaTick / 2 + obMPdf$eps)
                    else
                    {
...
LV<-function(y,f, s, t){
    ...
    for (i in 0:thmax)
    {
        for (k in 0:thmax)
        {
            ...
            gvtemp = (i * obMPdf$lambdaMA + (1 - i) * obMPdf$lambdaMA * hresp) * 
                (w[t+1,min(y + 1, obMPdf$NY), f, s] - w[t+1,y,f,s] + obMPdf$SS[s] * obMPdf$deltaTick/2 - obMPdf$deltaTick*i)+
                (k*obMPdf$lambdaMB + (1 - k) * obMPdf$lambdaMB * hresm) * 
                (w[t+1,max(y - 1, 1), f,s]-w[t+1,y,f,s] +  obMPdf$SS[s] * obMPdf$deltaTick/ 2 - obMPdf$deltaTick* k)
    ...
    lv = w[t+1,y,f,s] + obMPdf$deltat * obMPdf$YY[y] * Edp + obMPdf$deltat * Ls - 
